...
const Header = () => {
  ...
  const getTrendingMovies = async () => {
    ...
    const trendingContent = await axios.get("/trending/all/day");
    console.log('trendingContent getting fetched', trendingContent.data)
    const combinedData = [...data.results, ...trendingContent.data.results]
    console.log(combinedData,'Combined DATA GETTING FETCHED')


    let randomData =
      combinedData[Math.floor(Math.random() * data.results.length)];
    console.log('Random Data',trending);
  ...
  return (
    ...
    <>
      <div className="Header lg:w-full h-[50vh] overflow-hidden mt-2  bg-red-600 relative flex flex-col justify-center items-center"
      style={{
        backgroundImage: `url(${
          trending.backdrop_path
            ? `https://image.tmdb.org/t/p/original${
                trending.backdrop_path || trending.backdrop_path
              }`
      ...
      <div className="img-title  w-full ml-10">
        <div className=" lg:m-[0] m-[3rem] lg:mt-[3rem] flex flex-col  text-white justify-start ">
          ...
          <p className="lg:max-w-[80%] w-[100%] text-lg  block">
            {trending.overview ? trending.overview.slice(0, 160) : "Loading..."}{" "}
            ...more
          </p>

          <p className="text-white flex gap-4 lg:mt-5 items-center mt-8 lg:mb-0">
            <i className="ri-volume-down-line lg:text-[25px] text-[20px] font-bold text-yellow-500  lg:ml-2 text-nowrap">
              <span className="text-white">{trending.release_date}</span>
            </i>
            <i className="ri-movie-line text-xl font-bold lg:ml-2 text-yellow-500  text-[25px] text-nowrap">
              <span className="text-white lg:text-[25px] text-[20px]">
                {trending.popularity}
              </span>
            </i>
          </p>
